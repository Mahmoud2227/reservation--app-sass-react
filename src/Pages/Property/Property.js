...
const Property = () => {
	...
	const handleArrow = (direction) => {
		let index;
		if (direction === "left") {
			index = slideIndex === 0 ? 5 : slideIndex - 1;
		} else {
			index = slideIndex === 5 ? 0 : slideIndex + 1;
		}
		setSlideIndex(index);
	};
	return (
		<>
			...
			<div className='property'>
				{isOpen && (
					<div className='property__image-slideshow'>
						<div className='property__image-slideshow-wrapper'>
							<img src={images[slideIndex].src} alt='slider' />
							<FontAwesomeIcon
								icon={faCircleXmark}
								className='property__image-slideshow-wrapper-closeIcon'
								onClick={() => setIsOpen(false)}
							/>
							<FontAwesomeIcon
								icon={faCircleArrowRight}
								className='property__image-slideshow-wrapper-rightIcon'
								onClick={() => handleArrow("right")}
							/>
							<FontAwesomeIcon
								icon={faCircleArrowLeft}
								className='property__image-slideshow-wrapper-leftIcon'
								onClick={() => handleArrow("left")}
							/>
						</div>
					</div>
				)}
				<div className='property__container section__padding'>
					...
					<div className='property__info'>
						<div className='property__info-description'>
							<p>
								...
								from Miami Beach, Apartment Panorama Beach Montazah Families only provides
								beachfront accommodations with a bar, a terrace and free WiFi. With garden views,
								this property has a balcony.
							</p>
							<p>
							...
							<p>
								Alexandria Zoo is 9.7 km from the apartment, while Catacombs of Kom el Shoqafa is
								14.5 km away. The nearest airport is Borg El Arab International, 49.9 km from
								Apartment Panorama Beach Montazah Families only, and the property offers a paid
